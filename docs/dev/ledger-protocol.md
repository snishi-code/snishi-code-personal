...
- `schemaVersion`: スキーマ版。現行は **`7`**（v1→v2 `allocations`、v2→v3
  `cashflowSchedules`/`reserves`、v3→v4 `tags`、v4→v5 残高補正 `metadata.adjustment`
  永続化＝恒等移行、v5→v6 勘定科目に `role`、v6→v7 月額化コスト `monthlyCostItems` を追加）。
...
### `MonthlyCostItem`（月額化コスト）

サブスク・年払い・耐久財・定期イベントを統一して「月あたりの生活コスト」で見る登録簿。
生活コストは formula で導出する。**現金・預金払い・登録のみは仕訳を作らない**（計画/可視化レイヤ。
会計の正本は仕訳）。**負債払いで返済を入力したときだけ**、購入仕訳
`借方 按分中資産(deferred) / 貸方 支払用負債`（`metadata.monthlyCostId` 付き・費用にしない）で
負債を BS に立て、返済予定 CF を作る。
...
`endMonth?` / `expenseAccountId`(role expense-category) / `paymentAccountId?`(daily-asset |
payment-liability) / `repaymentAccountId?`(daily-asset) / `sourceAllocationId?` /
...
- version を上げるときは `SCHEMA_VERSION` を +1 し、`STEPS` に旧→新の手順を追加する。
  ...
  schema v5（`metadata.adjustment`）と v6（`account.role`）は既存ストア内の項目追加のみのため
  `DB_VERSION` は据え置き、schema v7 で新ストア追加に伴い `DB_VERSION` を 5 へ上げる。
