...
export const s = {
  ...
  detail: {
    ...
    // ページ (対象の詳細画面) で使うテンプレートの切替。
    templateAria: 'このページで使うテンプレート',
  ...
  settings: {
    ...
    // 設定: グループ (一覧・改名・削除・追加をこの場で直接行う。切替はホームのヘッダー)
    ward: {
      hint: 'グループの一覧です。タップで名前を変更（対象側の表示も自動で変わります）、プルダウンでそのグループのデフォルトテンプレート（新しいページに使うもの）を変更、ゴミ箱で削除（現在のグループ・対象が残っているグループは削除不可）、下のボタンで追加できます。グループの切り替えはホームのヘッダーで行います。',
      current: '現在のグループ',
      patientCount: (n: number) => `対象 ${n}件`,
      templateAria: (name: string) => `${name} のデフォルトテンプレート`,
      // 行そのものが名前編集の入口 (鉛筆は廃止)。読み上げでは対象名が要る。
      renameAria: (name: string) => `${name} の名前を変更`,
    },
