...
export function buildBuilderPrompt(sources: readonly string[], requestId: string): string {
  ...
  blocks.push(
    [
      '## 読み取り方の原則（最初に読むこと）',
      '- 人が読んで分かるように書かれた文章は、大きなまとまりの中に小さなまとまりが入る形になっている。',
      '- 大きなまとまりを「場所」、その中の小さなまとまりを「フォーマット」として、2 段の入れ子で読み取ること。',
      '- まとまりの区切り方は書き手によって違う。見出し記号・番号・空行・字下げ（インデント）・',
      '  罫線・大文字だけの行など、どれで区切られていてもよい。記号の有無で判断しないこと。',
      '- 表記の揺れは同じものとして扱うこと（半角スペースと全角スペース、":" と "："、',
      '  "・" と "-"、括弧の全角半角、行末の余分な空白など）。揺れを理由に項目を分けないこと。',
    ].join('\n'),
  );
  ...
  blocks.push(
    [
      '## 場所 (sections) の切り出し方',
      '- 見本に共通して現れる大きなまとまりを場所にすること。',
      '- 見出しがある場合（例: (S)、【概要】、■結果、1. 所見）は、その行を場所の区切りとみなすこと。',
      '- 見出し記号が無く、空行や字下げだけでまとまりが分かれている文章もある。その場合も',
      '  同じようにまとまりごとに場所を作ること。',
      '- title は見本の表記を記号・括弧込みで逐語的に書き写し、作り変えないこと。',
      '  見出しの行が無いまとまりは title を空文字列にすること（内容から名前を創作しないこと）。',
      '- まとまりが 1 つも見分けられなければ、場所を 1 つだけ作り title は空文字列にすること。',
  ...
  blocks.push(
    [
      '## フォーマットと項目の作り方',
      '- 場所の中で「ラベルと値」が毎回並ぶ小さなまとまりを 1 つのフォーマットにすること。',
      '- ラベルと値の区切りは書き手によって違う（コロン、スペース、タブ、全角スペースなど）。',
      '  区切り文字が何であってもラベルと値として読み取ること。',
      '- 1 つの場所の中に、書き方の違うまとまりが 2 つ以上あるときは、別々のフォーマットにすること',
      '  （例: 1 行に区切り文字で並ぶまとまりと、1 行 1 項目で並ぶまとまりは別のフォーマット）。',
