...
/*
 * テンプレートの適用範囲の用語（実ユーズレビュー 2026-08-14 / 2026-08-20 改定）:
 *   アプリ   = AppSettings.defaultTemplateId（内部フォールバック。UI からは設定しない —
 *              テンプレート一覧の行タップは 2026-08-20 に編集へ変更）
 *   グループ = PlaceDef.templateId（グループ作成時に 1 つ上のグループのものを写す。
 *              グループ 0 件時のみアプリのフォールバックを使う）
 *   ページ   = Patient.templateId（対象の詳細入力画面。作成時にグループのデフォルトを写す）
 * デフォルトは「作成時に写す」方式で、後からグループの設定を変えても
 * 既存のページには波及しない（それぞれの設定で個別に変える）。
 */
