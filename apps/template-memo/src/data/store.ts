...
/** 旧 workspace の変換済みデータを、既存データを置換せず 1 tx で末尾へ追記する。 */
export async function appendImported(data: WorkspaceImportPayload): Promise<void> {
  ...
  const settings = {
    ...state.settings,
    snippets: [...state.settings.snippets, ...prepared.snippets],
    updatedAt: Date.now(),
  };
  await db.runWrite([STORE_GROUPS, STORE_SUBJECTS, STORE_SETTINGS], (tx) => {
    for (const group of prepared.groups) tx.objectStore(STORE_GROUPS).put(group);
    for (const subject of prepared.subjects) tx.objectStore(STORE_SUBJECTS).put(subject);
    tx.objectStore(STORE_SETTINGS).put(settings);
  });
  notify({
    groups: [...state.groups, ...prepared.groups],
    subjects: [...state.subjects, ...prepared.subjects],
    settings,
  });
}
...
export async function restoreSubject(id: string, nowMs = Date.now()): Promise<void> {
  ...
  const groupId =
    cur.groupId && state.groups.some((g) => g.id === cur.groupId) ? cur.groupId : null;
  const peers = subjectsInGroup(groupId);
  const maxOrder = Math.max(0, ...peers.map((x) => x.sortOrder));
...
async function snapshotAndClear(reason: 'start' | 'clear', nowMs: number): Promise<void> {
  ...
  const round = reason === 'start' ? { startedAt: nowMs, endedAt: null } : state.settings.round;
