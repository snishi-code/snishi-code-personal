...
import { AdjustmentEditSheet } from '../src/ui/sheets/AdjustmentSheet';
import { OpeningEditSheet } from '../src/ui/sheets/OpeningSheet';
...
describe('金額以外のフィールドの open→save 往復', () => {
  ...
  it('台帳設定: 名前・単位・桁数を読み込んで保存し直しても export が通る', async () => {
    ...
    expect(exportToJsonText(await loadLedger())).toContain('"schemaVersion": 14');
  });
});
