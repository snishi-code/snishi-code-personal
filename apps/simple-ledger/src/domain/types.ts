...
export interface JournalEntry {
  ...
  /**
   * 諸口（複数フロー行の束）のグループ ID（2026-08-11 設計合意・グループ ID 方式。
   * v12 で予約・**v13.16 で本実装**）。振り分け保存（createEntries）だけが値を入れ、
   * 集計はどこも読まない（一覧の目印チップのみ）。検証は形式のみで、グループに
   * 「2 行以上」等の不変条件は将来も持たせない（1 行に減ったら普通の仕訳に退化する設計）。
   */
  groupId?: string;
  createdAt: string;
  updatedAt: string;
