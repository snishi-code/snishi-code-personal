/*
 * 資産の 3 グループ（自由に動かせるお金 / 自由に動かせないお金 / 月割り台帳）。
 *
 * ホームの資産ドリル（`Breakdown` の 3 枠）と、時間平面の数値レンズの資産行の展開
 ...
 * 壊さないための取り決め（schema 上 asset の role は 2 種だが、既定を明示しておく）。
 *
 * **ここは分類だけを持つ。並び（3 グループをどの順で出すか）は `domain/displayOrder` の
 * 箱の並びが正本**（`ASSET_GROUP_KEYS`）。分類と並びを 2 か所で書き分けないための役割分担。
 */
import type { MessageKey } from '../i18n';
import type { Account } from './types';

export type AssetGroupKey = 'free' | 'fixed' | 'ledger';
