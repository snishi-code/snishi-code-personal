...
test('ヘッダー期間を変えると仕訳一覧も追従する', async ({ page }) => {
  await page.goto('./');
  ...
  // ヘッダー期間を翌年（データなし）に変えると、当月の仕訳は範囲外になり消える。
  await page.locator(ui('period.button')).click();
  await page.locator(ui('period.input.year')).selectOption({ index: 0 }); // 先頭=最も新しい年（翌年）
  await page.locator(ui('period.grain.fullYear')).click();
  await page.keyboard.press('Escape');
  // 日付範囲がヘッダー期間（翌年）に追従し、当月の仕訳は一覧から消える。
