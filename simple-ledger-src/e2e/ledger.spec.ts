...
/**
 * 支出入力から継続コスト（資産経由）を 1 件作る。
 * 行き先の「継続コスト化」ボタンで対象名を自由入力 → 月数 → 分類先カテゴリ。
 */
async function addContinuousCost(
  page: Page,
  opts: {
    item: string;
    amount: string;
  ...
  await page.locator(ui('dashboard.entry.expense')).click();
  await page.locator(ui('journal.entry.item')).fill(opts.item);
  await pick(page, 'journal.entry.flow.source', opts.source);
  await page.locator(ui('journal.entry.amount')).fill(opts.amount);
  await page.locator(ui('journal.entry.ccToggle')).click();
  await page.locator(ui('journal.entry.ccName')).fill(opts.name);
  await pick(page, 'journal.entry.ccCategory', opts.category);
  await page.locator(ui('journal.entry.allocateMonths')).fill(opts.months);
  if (opts.continueBuy) await page.locator(ui('journal.entry.monthlyizeContinue')).check();
}
...
/** 勘定科目は「補正・勘定科目」(ハンバーガーメニューに昇格) に統合されている。 */
async function openAccountsManage(page: Page) {
  await page.locator(ui('nav.menu.button')).click();
...
test('継続コストを作成すると一覧と支出に反映される', async ({ page }) => {
  ...
  // 勘定科目の聖域化: 対象名も内部集約口座も勘定科目一覧に出ない。
  await openAccountsManage(page);
  await expect(page.locator(ui('accounts.view'))).toBeVisible();
  await expect(page.locator(ui('accounts.list'))).not.toContainText('ノートPC');
  await expect(page.locator(ui('accounts.list'))).not.toContainText('継続コスト台帳');
...
test('継続コストを後から編集すると一覧とホームの支出に反映される', async ({ page }) => {
  ...
  // 認識月数（過去に影響する項目）を変えたので「過去から再計算」の注意が出る。
  await expect(page.locator(ui('allocations.edit.impactWarning'))).toBeVisible();
...
test('返済を資金繰りに入れるON で返済元/回数が空なら保存できずエラー', async ({ page }) => {
  await page.goto('./');
  await addContinuousCost(page, {
    item: 'テレビ',
    amount: '120000',
  ...
  // 返済トグル ON のまま、返済元口座・回数を未入力で保存 → 必須エラーで止まる（静かに CF を作らない）。
  await page.locator(ui('journal.entry.monthlyizeRepayToggle')).check();
  await page.locator(ui('journal.entry.save')).click();
  await expect(page.getByText('返済回数は 1 以上で入力してください。')).toBeVisible();
  // モーダルは開いたまま（保存されていない）。
...
test('YouTube年払い（継続購入）: クレカ→YouTube資産化、当月認識1,000・資産は未認識11,000', async ({
  ...
  // 資産の内訳: 品目別ではなく集約台帳口座「継続コスト台帳」= 未消化残高 11,000
  // （資産化 12,000 − 当月認識 1,000）。個別品目名は BS に出さず 1 行に集約する。
  await page.locator(ui('dashboard.stat.assets')).click();
  await expect(page.locator(ui('assetsBreakdown.view'))).toContainText('継続コスト台帳');
  await expect(page.locator(ui('assetsBreakdown.view'))).not.toContainText('YouTube');
...
test('自動車ローンで自動車を買う: ローン→自動車→固定費 の三層 + 返済CF', async ({ page }) => {
  ...
  await page.locator(ui('journal.entry.ccToggle')).click();
  await page.locator(ui('journal.entry.ccName')).fill('自動車');
  await pick(page, 'journal.entry.ccCategory', '固定費');
  await page.locator(ui('journal.entry.allocateMonths')).fill('60');
  // 返済（ローン）→ 預金から 60 回返済。
  await page.locator(ui('journal.entry.monthlyizeRepayToggle')).check();
  await pick(page, 'journal.entry.monthlyizeRepayAccount', '預金');
  await page.locator(ui('journal.entry.monthlyizeRepayCount')).fill('60');
  await page.locator(ui('journal.entry.save')).click();

  // 3) 継続コスト台帳に「自動車」が出る。
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.allocations')).click();
  await expect(page.locator(ui('allocations.list'))).toContainText('自動車');

  // 4) 資金繰りに返済予定（自動車 返済）が出る。
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.cashflow')).click();
  await expect(page.locator(ui('cashflow.schedule.list'))).toContainText('自動車 返済');

  // 5) ホーム「支出」は購入額 2,500,000 ではない（月割り認識のみ＝返済は支出でなく CF）。
  await page.locator(ui('nav.home')).click();
  await expect(page.locator(ui('dashboard.stat.expense'))).not.toContainText('2,500,000');
});
...
test('振替の移動先を「取り置き資産を作る」で名称入力に切替えて作成でき、勘定科目は増えない', async ({
  page,
}) => {
  await page.goto('./');
  await page.locator(ui('dashboard.entry.transfer')).click();
  // 先に源泉（移動元）を選ぶ。移動先(右辺)を「取り置き資産を作る」で名称入力へ切替（継続コスト化と同じ）。
  await pick(page, 'journal.entry.flow.source', '預金');
  await page.locator(ui('journal.entry.reserveCreate')).click();
  await page.locator(ui('journal.entry.reserveName')).fill('新婚旅行');
  ...
  // 聖域化: 取り置き資産の目的名は勘定科目一覧に出ない（reserve-asset は内部ロール）。
  await openAccountsManage(page);
  await expect(page.locator(ui('accounts.list'))).not.toContainText('新婚旅行');

  // 勘定科目を増やさない＝資産内訳では集約「取り置き資金」1 行 + 目的別の入れ子（うち 新婚旅行）。
  await page.locator(ui('nav.home')).click();
  await page.locator(ui('dashboard.stat.assets')).click();
  const assets = page.locator(ui('assetsBreakdown.view'));
  await expect(assets).toContainText('取り置き資金');
  await expect(assets.locator(ui('assetsBreakdown.reserveSub'))).toContainText('新婚旅行');
  await expect(assets.locator(ui('assetsBreakdown.reserveSub'))).toContainText('100,000');
});

test('取り置き資金は振替・支出の両辺で既定で選べる（チェックボックス無し）', async ({ page }) => {
  await page.goto('./');
  ...
  // 支出の支払い方法（source）にトグル無しで既定で出る。
  await page.locator(ui('nav.home')).click();
  await page.locator(ui('dashboard.entry.expense')).click();
  await expect(page.locator(ui('journal.entry.flow.source'))).toContainText('旅行積立');

  // 振替の移動元・移動先の両辺でも既定で出る。
  await page.locator(ui('journal.entry.cancel')).click();
  await page.locator(ui('dashboard.entry.transfer')).click();
  await expect(page.locator(ui('journal.entry.flow.source'))).toContainText('旅行積立');
  await expect(page.locator(ui('journal.entry.flow.destination'))).toContainText('旅行積立');
});

test('取り置き資産はシンプル: 作成は名前のみ、一覧は残高のみ（目標額・期限・必要積立額を持たない）', async ({
  page,
}) => {
  await page.goto('./');
  await page.locator(ui('nav.menu.button')).click();
  ...
  // 作成シートは名前のみ。目標額・目標期限の入力欄は出さない（短期の封筒分け）。
  await expect(page.locator(ui('cashflow.reserve.name'))).toBeVisible();
  await expect(page.locator(ui('cashflow.reserve.target'))).toHaveCount(0);
  await expect(page.locator(ui('cashflow.reserve.date'))).toHaveCount(0);

  await page.locator(ui('cashflow.reserve.name')).fill('飲み会用');
  await page.locator(ui('cashflow.reserve.save')).click();

  // 一覧は名前 + 残高 + 削除のみ。必要積立額・目標は出さない。
  const list = page.locator(ui('cashflow.reserve.list'));
  await expect(list).toContainText('飲み会用');
  await expect(list).not.toContainText('必要');
  await expect(list).not.toContainText('目標');
});

test('継続コスト入力の語彙・導線: 分類先カテゴリ／ローンを組む（認識・ローン等・新しいカードを出さない）', async ({
  page,
}) => {
  await page.goto('./');
  await page.locator(ui('dashboard.entry.expense')).click();
  ...
  await expect(
    page.locator(ui('journal.entry.flow')).locator(ui('journal.entry.loanArrange')),
  ).toBeVisible();
  ...
  await expect(page.getByText('新しいカードを作る')).toHaveCount(0);
  // 既定では新規ローン作成は畳まれ、「ローンを組む」を押すと左辺が切り替わり作成導線が出る。
  await expect(page.getByText('新しいローンを作成')).toHaveCount(0);
  await page.locator(ui('journal.entry.loanArrange')).click();
...
test('日常入力（収入/支出/振替）はホーム下部の固定バー内にあり、スクロールしても残る', async ({
  page,
}) => {
  await page.goto('./');
  const bar = page.locator(ui('dashboard.entryBar'));
  await expect(bar).toBeVisible();
  // 3ボタンが固定バーの中にある。
  await expect(bar.locator(ui('dashboard.entry.income'))).toBeVisible();
  await expect(bar.locator(ui('dashboard.entry.expense'))).toBeVisible();
  await expect(bar.locator(ui('dashboard.entry.transfer'))).toBeVisible();
  // 下部固定（position: fixed）= スクロールしても画面下に残る。
...
test('下部固定バーの「支出」から入力シートが中央dialogとして開く', async ({ page }) => {
  await page.goto('./');
  const bar = page.locator(ui('dashboard.entryBar'));
  await bar.locator(ui('dashboard.entry.expense')).click();
  // 入力フォーム（dialog）が開く。下からせり上がる下部シートではなく中央配置。
...
test('支出サマリーから支出内訳へ行くと費用カテゴリ別の内訳が見える（合計はホームと一致）', async ({
  page,
}) => {
  await page.goto('./');
  // 通常支出（変動費 2,500）+ 継続コスト（冷蔵庫 240,000/48か月=5,000/月 を固定費へ）。
  await addExpense(page, '食費', '2500');
  await addContinuousCost(page, {
    item: '冷蔵庫',
    amount: '240000',
    source: '現金',
  ...
  // 主表示は費用カテゴリ別内訳（変動費 / 固定費）。
  const list = page.locator(ui('expenseBreakdown.categoryList'));
  await expect(list).toBeVisible();
  await expect(list).toContainText('変動費');
  await expect(list).toContainText('固定費');
  // 継続コストの月割り 5,000 は認識先の固定費カテゴリに合算される。
  await expect(
    page.locator(ui('expenseBreakdown.categoryRow')).filter({ hasText: '固定費' }),
  ).toContainText('5,000');
  // 合計はホーム「支出」と一致（2,500 + 5,000 = 7,500）。
  await expect(page.locator(ui('expenseBreakdown.total'))).toContainText('7,500');
});

test('収支ページの 収入/支出 カードはホームの同名ボタンと同じ内訳ページへ遷移する', async ({
  page,
}) => {
  await page.goto('./');
  // ホーム「収支」→ 収支ページ。
  ...
  await expect(page.locator(ui('netIncome.view'))).toBeVisible();
  // 収入カード → 収入の内訳。
  await page.locator(ui('netIncome.revenue')).click();
  await expect(page.locator(ui('incomeBreakdown.view'))).toBeVisible();
  // 収支へ戻り、支出カード → 支出の内訳。
  await page.locator(ui('nav.home')).click();
  await page.locator(ui('dashboard.stat.netIncome')).click();
  await page.locator(ui('netIncome.expense')).click();
  await expect(page.locator(ui('expenseBreakdown.view'))).toBeVisible();
});
