...
test('ホーム上段の「支出」は購入額でなく支出（継続コスト分）を表示する', async ({ page }) => {
  await page.goto('./');
  // 冷蔵庫 240,000 を耐久財・固定資産として 48 か月で継続コスト（現金払い）。
  await page.locator(ui('dashboard.entry.expense')).click();
...
test('支出の継続コストで「耐久財・固定資産」を選ぶと固定資産化され、売却/故障できる（洗濯機/カード/84か月/2回）', async ({
  page,
}) => {
  await page.goto('./');
  await page.locator(ui('dashboard.entry.expense')).click();
  ...
  // 継続コスト一覧に洗濯機が出て、固定資産由来なので「売却/故障」ボタンがある。
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.allocations')).click();
...
test('継続コストは実支払いを仕訳に残し、当月の月割り認識も見える', async ({ page }) => {
  await page.goto('./');
  await page.locator(ui('dashboard.entry.expense')).click();
  ...
  await openJournal(page);
  // 実際の支払い（120,000）は仕訳に残る（読み取り専用の「継続コスト」タグ付き）。
