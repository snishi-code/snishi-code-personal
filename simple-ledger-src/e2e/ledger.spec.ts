...
test('?fixture=sample で空DBを開くと最初からサンプルデータが入っている', async ({ page }) => {
  ...
  // 月額化コスト一覧にサンプルの「家賃」が入っている（fixture が読み込まれた証拠・期間非依存）。
  await page.locator(ui('nav.menu.button')).click();
...
test('通常起動（fixture 指定なし）では空の台帳で始まる（サンプルを勝手に入れない）', async ({
  ...
  await page.goto('./');
  await expect(page.locator(ui('dashboard.view'))).toBeVisible();
  // サンプルの月額化コスト「家賃」は入っていない（fixture を勝手に投入しない）。
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.allocations')).click();
...
test('ホーム上段の「支出」は購入額でなく生活コスト（月額化分）を表示する', async ({ page }) => {
  await page.goto('./');
  // 冷蔵庫 240,000 を耐久財・固定資産として 48 か月で月額化（現金払い）。
  await page.locator(ui('dashboard.entry.expense')).click();
  ...
  await page.locator(ui('journal.entry.allocateAssetKind')).selectOption('fixed');
  await page.locator(ui('journal.entry.allocateMonths')).fill('48');
  await page.locator(ui('journal.entry.save')).click();

  // ホーム上段「支出」は購入額 240,000 ではなく、当月の月額化分 5,000（生活コスト）を表示する。
  await page.locator(ui('nav.home')).click();
...
test('月額化コストを後から編集すると一覧とホームの生活コストに反映される', async ({ page }) => {
  await page.goto('./');
  ...
  // 月額化コスト画面で編集（名称変更 + 期間 48 → 24 か月 = 月額 10,000）。
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.allocations')).click();
  ...
  // ホームの生活コストも 10,000 に反映される。
  await page.locator(ui('nav.home')).click();
  await expect(page.locator(ui('dashboard.view'))).toContainText('10,000');
});

test('固定資産由来の月額化コストを売却/故障で処分でき、終了する', async ({ page }) => {
  await page.goto('./');
  // 1) 固定資産科目「社用車」を作る。
  ...
  // 2) 支出で固定資産購入を「固定資産として月額化」する（120 か月）。
  await page.locator(ui('nav.home')).click();
  await page.locator(ui('dashboard.entry.expense')).click();
  ...
  await expect(page.locator(ui('allocations.view'))).toContainText('月額化コストはありません');
  await page.locator(ui('allocations.showCompleted')).check();
  await expect(page.locator(ui('allocations.list'))).toContainText('社用車の購入');
  await expect(page.locator(ui('allocations.list'))).toContainText('終了');
});

test('支出の月額化で「耐久財・固定資産」を選ぶと固定資産化され、売却/故障できる（洗濯機/カード/84か月/2回）', async ({
  page,
}) => {
  await page.goto('./');
  await page.locator(ui('dashboard.entry.expense')).click();
  ...
  await page.locator(ui('journal.entry.amount')).fill('240000');
  await page.locator(ui('journal.entry.allocateToggle')).check();
  // 種類 = 耐久財・固定資産（固定資産科目を事前に作らずに正規ルートへ）。
  await page.locator(ui('journal.entry.allocateAssetKind')).selectOption('fixed');
  ...
  // 月額化コスト一覧に洗濯機が出て、固定資産由来なので「売却/故障」ボタンがある。
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.allocations')).click();
  await expect(page.locator(ui('allocations.list'))).toContainText('洗濯機');
  await expect(page.locator(ui('allocations.dispose')).first()).toBeVisible();
...
test('ホーム「支出」→ 支出の内訳 → 月額化コスト台帳の導線', async ({ page }) => {
  await page.goto('./');
  // 「支出」は月額化コスト台帳へ直行せず、まず「支出の内訳」へ。
  ...
  // 生活コスト（月額化コスト）からだけ月額化コスト台帳へ進める。
  await page.locator(ui('expenseBreakdown.monthlyCost')).click();
  await expect(page.locator(ui('allocations.view'))).toBeVisible();
});

test('ハンバーガーメニューから月額化コスト台帳へ直接行ける', async ({ page }) => {
  await page.goto('./');
  await page.locator(ui('nav.menu.button')).click();
  await page.locator(ui('nav.allocations')).click();
  await expect(page.locator(ui('allocations.view'))).toBeVisible();
...
test('ヘッダーは期間コンテキスト表示で、軽量ピッカーから年/月/年全体/全期間を切り替える', async ({
  ...
  // 生活コストは独立セクションにしない（上段「支出」がその値）。
  await expect(page.locator('.section-label').filter({ hasText: '生活コスト' })).toHaveCount(0);
