...
describe('起動時 schemaVersion 追従', () => {
  ...
  it('既存DBの v14 追従: 品目別 continuing-cost-asset 科目を集約台帳口座へ寄せる（品目名は残る）', async () => {
    const init = await loadLedger();
    // v13 時点の DB を模す: 品目別の continuing-cost-asset 科目 2 件 + それを指す台帳項目 2 件。
    ...
    await putKv('meta', { ...init.meta, schemaVersion: 13 });
    ...
    expect(after.accounts.some((a) => a.id === 'cc-washer' || a.id === 'cc-youtube')).toBe(false);
    // 品目名は残り、recognitionCreditAccountId は集約口座へ付け替わる。
    const items = after.monthlyCostItems.filter((m) => m.id === 'm1' || m.id === 'm2');
...
describe('保存境界の fail-closed（構造・参照検証 + i18n エラーコード）', () => {
  ...
  it('createReserve を複数回呼んでも reserve-asset 科目は集約口座 1 件だけ', async () => {
    ...
    await createReserve({ name: '車の頭金', targetAmount: 3000000 });
    const after = await loadLedger();
