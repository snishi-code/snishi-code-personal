...
// foundation は i18n を持たないので、失敗理由を reason コードで返し、
// 表示文言はアプリ側に任せる。
...
export interface RouteRejection {
  ...
  // consumed:false = 入力欄を消してはいけない (fail-closed)
  consumed: false;
...
// allowedKinds: この入口で受け付ける kind の allowlist (アプリ方針)。
// 省略時は登録済み kind をすべて受け付ける。
